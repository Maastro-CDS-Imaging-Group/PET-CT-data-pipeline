...
class PatchAggregator3D():
    ...
    def aggregate(self, patches_list):
        ...
        for i, patch in enumerate(patches_list):
            ...
            if self.overlap_handling == 'intersection':
                full_volume_copy = full_volume.copy()
                full_volume[z1:z2, y1:y2, x1:x2] = patch
                full_volume = full_volume * full_volume_copy
...
if __name__ == '__main__':
    ...
    # Patches have (D,H,W) order internally
    patches_list = [np.zeros(patch_size[2], patch_size[1], patch_size[0]) for i in range(8)]
